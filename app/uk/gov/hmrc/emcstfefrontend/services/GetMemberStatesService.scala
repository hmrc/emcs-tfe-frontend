...
import uk.gov.hmrc.emcstfefrontend.models.response.MemberStatesException
...
class GetMemberStatesService @Inject()(connector: GetMemberStatesConnector)
  ...
  def getMemberStates()(implicit hc: HeaderCarrier): Future[Seq[MemberState]] = {
    connector.getMemberStates().map {
      case Left(_) => throw MemberStatesException("No member states retrieved")
      case Right(memberStates) => memberStates
    }
  }
}