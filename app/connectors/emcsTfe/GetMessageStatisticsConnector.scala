    ...
    get(url)
      .recover {
        case JsResultException(errors) =>
          logger.warn(s"[getMessageStatistics][$exciseRegistrationNumber] Bad JSON response from emcs-tfe: " + errors)
          Left(JsonValidationError)
        case error =>
          logger.warn(s"[getMessageStatistics][$exciseRegistrationNumber] Unexpected error from emcs-tfe: ${error.getClass} ${error.getMessage}")
