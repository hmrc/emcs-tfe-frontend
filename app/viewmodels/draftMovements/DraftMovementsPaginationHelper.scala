  ...
  def constructPagination(pageCount: Int, ern: String, search: GetDraftMovementsSearchOptions): Option[Pagination] = {
    ...
    if (pageCount == 1) None else {
      ...
      val firstItem: Option[PaginationItem] = if (search.index <= 1) None else Some(PaginationItem(
        href = routes.ViewAllDraftMovementsController.onPageLoad(ern, search.copy(index = 1)).url,
        number = Some("1")
      ))
      ...
      val currentItem: Option[PaginationItem] = Some(PaginationItem(
        href = routes.ViewAllDraftMovementsController.onPageLoad(ern, search.copy(index = search.index)).url,
      ...
      val lastItem: Option[PaginationItem] = if (search.index >= pageCount) None else Some(PaginationItem(
        href = routes.ViewAllDraftMovementsController.onPageLoad(ern, search.copy(index = pageCount)).url,
