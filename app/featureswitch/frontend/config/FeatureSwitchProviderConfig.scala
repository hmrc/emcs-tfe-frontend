...
class FeatureSwitchProviderConfig @Inject()(configuration: Configuration) {
  ...
  private lazy val emcsTfeFrontendFeatureSwitchUrl =
    s"${servicesConfig.baseUrl("emcs-tfe-frontend")}/emcs/account/test-only/api/feature-switches"

  private lazy val emcsTfeFeatureSwitchUrl =
    s"${servicesConfig.baseUrl("emcs-tfe")}/emcs-tfe/test-only/api/feature-switches"

  private lazy val selfFeatureSwitchProvider: FeatureSwitchProvider = FeatureSwitchProvider(
    id = "emcs-tfe-frontend",
    appName = "EMCS-TFE Frontend",
    url = emcsTfeFrontendFeatureSwitchUrl
  )

  private lazy val emcsTfeFeatureSwitchProvider: FeatureSwitchProvider = FeatureSwitchProvider(
    id = "emcs-tfe",
