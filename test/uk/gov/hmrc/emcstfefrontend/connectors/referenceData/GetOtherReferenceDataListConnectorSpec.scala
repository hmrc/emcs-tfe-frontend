...
class GetOtherReferenceDataListConnectorSpec extends UnitSpec with Status with MimeTypes with HeaderNames with ModeOfTransportListFixture {

  trait Test extends MockAppConfig with MockHttpClient {
    implicit val hc: HeaderCarrier = HeaderCarrier()
    implicit val ec: ExecutionContext = app.injector.instanceOf[ExecutionContext]

    val connector = new GetOtherReferenceDataListConnector(mockHttpClient, mockAppConfig)
  ...
  "getOtherReferenceDataList" should {
    "return a successful response" when {
      "downstream call is successful" in new Test {
        ...
        await(connector.getOtherReferenceDataList()) shouldBe response
      }
    }
  }
}
